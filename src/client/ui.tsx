...
export const Icons = {
  ...
  /** microphone — 口述访谈 */
  mic: 'M8 1.8a2.2 2.2 0 0 1 2.2 2.2v3.5a2.2 2.2 0 0 1-4.4 0V4A2.2 2.2 0 0 1 8 1.8zM3.8 7.2a4.2 4.2 0 0 0 8.4 0M8 11.4V14M5.8 14h4.4',
  /** audio waveform — 音频/转录管线 */
  waveform: 'M2 8h1.4M5 4.5v7M8 2.5v11M11 5.5v5M14 8h-.4',
  /** reel-to-reel tape — 历史录音载体 */
  tape: 'M4.5 3.5h7a3 3 0 0 1 0 6h-7a3 3 0 0 1 0-6zM5.5 6.5h.01M10.5 6.5h.01M4.5 9.5l-1.5 3M11.5 9.5l1.5 3',
  /** archive box — 档案 */
  archive: 'M2 4.5h12v9H2zM1.5 2.5h13v2h-13zM6.5 7.5h3',
  /** scroll / manuscript — 手稿古籍 */
  scroll: 'M4 2.5h8v11H4zM4 2.5A1.5 1.5 0 0 0 2.5 4v8.5H4M6 5.5h4M6 8h4M6 10.5h2.5',
  /** timeline / chronology */
  timeline: 'M2 8h12M4.5 8V3.5M8 8V5.5M11.5 8v-3M4.5 13h7',
  /** quote mark — 引文 */
  quote: 'M4 6.5c0-1.5 1-2.5 2.5-3M4 6.5V10h3.5V6.5H4zM9.5 6.5c0-1.5 1-2.5 2.5-3M9.5 6.5V10H13V6.5H9.5z',
  /** verified / heard-checked badge — 人工听校 */
  verified: 'M8 1.8l1.9 1.4 2.3-.2.7 2.2 1.9 1.4-1 2.1 1 2.1-1.9 1.4-.7 2.2-2.3-.2L8 15.2l-1.9-1.4-2.3.2-.7-2.2L1.2 10.4l1-2.1-1-2.1 1.9-1.4.7-2.2 2.3.2L8 1.8zM6.2 8l1.4 1.4L10 6.6',
  /** warning / 存疑待考 */
  caution: 'M8 2.5l5.8 10.5H2.2L8 2.5zM8 6.5v3M8 11.3h.01',
} as const;
...
/** 考据卡状态配色（draft→corroborated→settled，contested/dropped 收尾） */
export function statusColor(status: string): string {
  switch (status) {
    case 'corroborated': return T.business;   // 已获旁证
    case 'settled': return T.success;         // 已成定论
    case 'contested': return T.danger;        // 存在争议
    case 'dropped': return T.caption;         // 已弃用
    default: return T.warning;                // draft 草稿
  }
}
...
export function categoryColor(kind: string): string {
  switch (kind) {
    case 'extract': return T.business;      // 史料摘录
    case 'chronology': return T.teal;       // 史实编年
    case 'concept': return T.purple;        // 概念辨析
    case 'exegesis': return T.success;      // 考释解读
    case 'dispute': return T.danger;        // 学术争议
    case 'thesis': return T.warning;        // 论文论点
    case 'context': return 'rgba(139,148,158,.75)'; // 时代背景
    default: return T.caption;
  }
}

/** 史料轨道/载体配色（史料库列表左侧色条） */
export function tierColor(tier: string, kind?: string): string {
  if (tier === 'secondary') return 'rgba(124,92,255,.75)';   // 二手研究统一紫
  switch (kind) {
    case 'oral-history': return T.business;  // 口述访谈——本工作台的主战场
    case 'archive': return T.warning;
    case 'manuscript': return T.teal;
    case 'rare-book': return '#b45309';
    case 'periodical': return '#0891b2';
    case 'patent': return T.success;
    case 'instrument': return '#c2410c';
    case 'photograph': return '#a855f7';
    case 'correspondence': return '#0ea5e9';
    case 'lab-notebook': return '#65a30d';
    default: return T.caption;
  }
}
...
export function segmentColor(status: string): string {
  switch (status) {
    case 'human-verified': return T.success;   // 已人工听校
    case 'ai-corrected': return T.business;    // AI 修正待核
    case 'uncertain': return T.warning;        // 存疑
    case 'inaudible': return T.danger;         // 无法辨听
    default: return T.caption;                 // raw 未校
  }
}
...
export function edgeColor(kind: string): string {
  switch (kind) {
    // 人物关系族（师承/同僚/论战/接任）
    case 'mentored': return T.business;
    case 'colleague': return 'rgba(13,148,136,.75)';
    case 'debated': return T.danger;          // 论战——必须最醒目
    case 'succeeded': return 'rgba(139,148,158,.7)';
    case 'patronized': return T.warning;
    // 机构归属族
    case 'affiliated': return 'rgba(139,148,158,.5)';
    case 'founded': return T.purple;
    // 技术与知识族（发明/改良/转移/本土化/理论化/应用）
    case 'invented': return T.success;
    case 'improved': return 'rgba(101,163,13,.8)';
    case 'transferred': return '#c2410c';     // 技术转移——物质流动
    case 'localized': return '#0891b2';       // 本土化改制
    case 'theorized': return T.purple;
    case 'applied': return 'rgba(13,148,136,.7)';
    // 文献族（浅灰刻意分层：文献学事实 vs 语义关系）
    case 'authored': return 'rgba(127,127,127,.55)';
    case 'cited': return 'rgba(127,127,127,.35)';
    case 'annotated': return 'rgba(180,83,9,.6)';
    // 时间与因果族（虚线渲染由视图负责）
    case 'preceded': return 'rgba(139,148,158,.4)';
    case 'caused': return 'rgba(229,72,77,.6)';
    case 'influenced': return 'rgba(124,92,255,.55)';
    // 卡片来源（琥珀，与卡片节点同色系）
    case 'derived_from': return 'rgba(245,165,36,.6)';
    case 'related': return 'rgba(139,148,158,.45)';
    default: return T.caption;
  }
}

/**
 * 史学关系族分类——谱系图谱用于分组力导向与图例。
 * 人物 / 机构 / 技术知识 / 文献 / 时间因果。
 */
export const EDGE_FAMILIES = {
  person: ['mentored', 'colleague', 'debated', 'succeeded', 'patronized'],
  institution: ['affiliated', 'founded'],
  technique: ['invented', 'improved', 'transferred', 'localized', 'theorized', 'applied'],
  document: ['authored', 'cited', 'annotated'],
  temporal: ['preceded', 'caused', 'influenced', 'derived_from', 'related'],
} as const;

/** 节点类型配色（谱系图谱） */
export function nodeColor(kind: string): string {
  switch (kind) {
    case 'person': return T.business;
    case 'institution': return T.purple;
    case 'artifact': return T.success;
    case 'event': return T.danger;
    case 'concept': return T.teal;
    case 'publication': return 'rgba(245,165,36,.85)';
    case 'place': return '#0891b2';
    case 'source': return 'rgba(127,127,127,.7)';
    case 'card': return 'rgba(180,83,9,.8)';
    default: return T.caption;
  }
}
