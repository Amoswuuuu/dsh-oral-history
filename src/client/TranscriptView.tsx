...
function AudioPlayer({ t, src, segList, attachAudio, currentTime, duration, playing, speed, error, onToggle, onSeek, onSpeed, onTimeUpdate, onDurationChange, onError }: {
  t: TFunc;
  src: string;
  segList: Segment[];
...
export function TranscriptView({ t }: { t: (key: string, params?: Record<string, unknown>) => string }) {
  ...
  const seek = useCallback((sec: number) => {
    const el = audioRef.current;
    if (!el) return;
    try { el.currentTime = Math.max(0, sec); } catch { /* 忽略未就绪 */ }
        ...
        <>
          ...
          <div ref={detailScrollerRef} className="sch-scroll" style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
            ...
            {audioSrc ? (
              ...
              <Section title={t('iv.audio')} icon={<Icon d={Icons.waveform} size={11} />}>
                <div style={{ fontSize: 11, color: T.caption, lineHeight: 1.6 }}>{t('tr.noAudioHint')}</div>
              </Section>
            )}
